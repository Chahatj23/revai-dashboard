...
import { 
  LayoutDashboard, Package, Users, Settings, LogOut, ShoppingCart, 
  Percent, Sparkles, Cable, Zap, ChevronRight, Building2, Target, 
  BarChart3, Activity 
} from "lucide-react";
      ...
      <div className="p-8 pb-4 flex items-center justify-between">
        <div className={cn("flex items-center gap-3", !isOpen && "justify-center w-full")}>
          ...
          {isOpen && (
            <div className="animate-in fade-in duration-300 whitespace-nowrap overflow-hidden">
              <h1 className="text-xl font-bold tracking-tight text-white">RevAI</h1>
              <p className="text-[10px] font-bold uppercase tracking-widest text-primary/80">Intelligence Hub</p>
            </div>
          )}
        </div>
      </div>
        ...
        {isOpen && currentUser?.organizationName && (
          <div className="mx-8 mb-6 p-3 rounded-xl bg-white/5 border border-white/5 flex items-center justify-between group cursor-default animate-in fade-in duration-300 whitespace-nowrap overflow-hidden">
            <div>
              <p className="text-[10px] font-black text-muted-foreground uppercase tracking-widest leading-none mb-1">Active Account</p>
              <h3 className="text-sm font-bold text-white truncate max-w-[120px]">{currentUser.organizationName}</h3>
            </div>
      ...
      <nav className="flex-1 px-4 py-2 space-y-8 overflow-y-auto overflow-x-hidden custom-scrollbar">
        {navItems.map((group) => (
          <div key={group.group} className="space-y-1">
             {isOpen && <p className="px-4 py-2 text-[10px] font-black text-muted-foreground uppercase tracking-[0.2em] opacity-40">{group.group}</p>}
                ...
                >
                  <div className="flex items-center gap-3">
                    <item.icon size={18} className={cn(
                      ...
                      location.pathname === item.href ? "text-white" : "text-primary/60"
                    )} />
                    {isOpen && <span className="font-semibold text-sm whitespace-nowrap animate-in fade-in">{item.label}</span>}
                  </div>
                  {isOpen && location.pathname === item.href && <ChevronRight size={14} className="opacity-50 shrink-0" />}
                </Link>
             ))}
          </div>
        ))}
      ...
      <div className="p-4 mt-auto border-t border-sidebar-border space-y-2 flex flex-col items-center">
        {isOpen ? (
            ...
            >
              ...
              <span className="whitespace-nowrap animate-in fade-in">System Settings</span>
            </Link>
        ) : (
             <Link 
              to={`/org/${orgId}/settings`}
        ...
        {isOpen ? (
            ...
            >
              ...
              <span className="whitespace-nowrap animate-in fade-in">Terminate Session</span>
            </button>
        ) : (
            <button 
              onClick={() => logout()}
        ...
        >
          <ChevronRight size={16} className={cn("transition-transform duration-300", isOpen && "rotate-180")} />
        </button>
      </div>
    </aside>
  );
    ...
    return (
      <div className="h-screen w-screen flex items-center justify-center bg-background">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-sm font-bold text-muted-foreground uppercase tracking-widest">Initialising Systems...</p>
        </div>
      </div>
    );
  }

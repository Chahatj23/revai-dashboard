      ...
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div className="space-y-1">
          <h1 className="text-5xl font-extrabold tracking-tighter text-white">
            RevAI Inventory Control
          </h1>
          <p className="subtitle text-primary font-bold uppercase tracking-widest text-xs">
            Live System Control & Analytics
          </p>
        </div>
        <div className="flex gap-4">
          <Button
            variant="outline"
            size="lg"
            className="glass-panel border-primary/20 hover:border-primary/50 text-white font-bold h-14 px-8"
          >
        ...
        {[
          {
            ...
            val: totalProducts,
            icon: Package,
            sub: "Unique product lines",
            color: "text-blue-400",
          },
          {
            label: "Total Stock Volume",
            val: totalStockUnits.toLocaleString(),
            icon: ListChecks,
            sub: "Total items in network",
            color: "text-purple-400",
          },
          {
            label: "Inventory Value",
            val: `$${totalInventoryValue.toLocaleString(undefined, { maximumFractionDigits: 0 })}`,
            icon: DollarSign,
            sub: "Estimated stock worth",
            color: "text-emerald-400",
          },
          {
            label: "Sales Revenue",
            val: `$${totalSalesValue.toLocaleString(undefined, { maximumFractionDigits: 0 })}`,
            icon: TrendingUp,
            sub: "Total paid sales",
            color: "text-orange-400",
          },
        ].map((kpi, i) => (
          <Card
            key={i}
            className="glass-panel border-none shadow-2xl hover:translate-y-[-4px] transition-all duration-300"
          >
              ...
              <div className="flex items-center gap-6">
                ...
                <div className="text-xs font-bold text-muted-foreground space-y-1 leading-tight">
                  ...
                  {lowStockProducts.length > 0 && (
                    <Button
                      variant="link"
                      className="p-0 h-auto text-destructive text-[10px] font-black uppercase tracking-widest"
                      asChild
                    >
                      <Link to="/inventory/manage?filter=low">
                        View Critical Nodes →
                      </Link>
                    </Button>
                  )}
            ...
            <CardContent className="grid grid-cols-1 gap-3">
              <Button className="justify-start gap-4 h-12 font-bold" asChild>
                <Link to="/products/add">
                  <Package className="h-4 w-4" /> New Product Node
                </Link>
              </Button>
              <Button
                variant="outline"
              ...
              >
                <Link to="/orders">
                  <ShoppingCart className="h-4 w-4" /> Record Trade Phase
                </Link>
              </Button>
              <Button
                variant="secondary"
              ...
              >
                <Link to="/promotions">
                  <TrendingUp className="h-4 w-4" /> Inject Promotion
                </Link>
              </Button>
            </CardContent>
          </Card>
            ...
            {loading ? (
              ...
              <div className="space-y-4">
                {lowStockProducts.slice(0, 3).map((product) => (
                  <div
                    key={product.id}
                    className="flex justify-between items-center p-5 rounded-2xl bg-white/5 border border-white/5 hover:bg-white/10 transition-all group"
                  >
                    <div className="space-y-1">
                      ...
                      <p className="text-[10px] font-black uppercase text-muted-foreground tracking-widest">
                        CRITICAL: {product.stockLevel} / RECOMMENDED:{" "}
                    ...
                    <Button
                      size="sm"
                      variant="ghost"
                      className="text-destructive font-black uppercase tracking-widest text-[10px] hover:bg-destructive/10"
                      asChild
                    >
              ...
              <div className="flex flex-col items-center justify-center py-12 text-center">
                ...
                <h4 className="font-black text-xl italic text-emerald-500/80 uppercase tracking-tighter">
                  Network Optimized
                </h4>
                <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest mt-2">
                  All product nodes maintain safe volume
                </p>
              </div>
            )}
          </CardContent>
